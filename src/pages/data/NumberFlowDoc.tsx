    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-muted-foreground">
          数字过渡组件，支持 Intl 格式化、前后缀、趋势和动态效果偏好。
        </p>
      </div>
...
import { Button, NumberFlow } from "@easyfix/console-ui";

const [value, setValue] = useState(431.1);

<NumberFlow value={value} size="xl" />
<Button size="sm" onClick={() => setValue((current) => current + 128.42)}>增加</Button>
<Button size="sm" variant="outline" onClick={() => setValue((current) => current - 56.8)}>
  减少
</Button>`}
      >
...
<NumberFlow
  ...
  size="lg"
/>
<Button size="sm" onClick={() => setValue((current) => current + 120)}>更新金额</Button>`}
      >
        <CurrencyDemo />
      </ComponentDemo>
      ...
      <ComponentDemo
        ...
        code={`<NumberFlow value={1280000} locales="vi-VN" format={{ style: "currency", currency: "VND" }} size="lg" />
<NumberFlow value={9824.56} locales="zh-CN" format={{ style: "currency", currency: "CNY" }} size="lg" />
<NumberFlow value={982456} locales="en-US" format={{ notation: "compact" }} suffix=" visits" />`}
      ...
      <ComponentDemo
        ...
        code={`const [revenue, setRevenue] = useState(124.23);
const [diff, setDiff] = useState(0.0564);

<NumberFlowGroup>
  <NumberFlow value={revenue} locales="en-US" format={{ style: "currency", currency: "USD" }} size="lg" />
  <NumberFlow
    value={diff}
    locales="en-US"
    format={{ style: "percent", maximumFractionDigits: 2, signDisplay: "always" }}
...
<Button onClick={() => { setRevenue((current) => current + 12.4); setDiff(0.072); }}>
  上升
</Button>
<Button variant="outline" onClick={() => { setRevenue((current) => current - 8.2); setDiff(-0.031); }}>
  下降
</Button>`}
      >
        <TrendDemo />
